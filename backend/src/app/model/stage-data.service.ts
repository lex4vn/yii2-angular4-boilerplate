...
export class StageDataService {
    ...
    addStage(stage:Stage):Observable<any>{
        ...
        return this._authHttp.post(
            this._globalService.apiHost+'/stage',
    ...
    // DELETE /v1/stage/1
    deleteStageById(id:number):Observable<boolean>{
        let headers = this.getHeaders();

        return this._authHttp.delete(
            this._globalService.apiHost+'/stage/'+id,
            {
                headers: headers
            }
        )
    ...
    // PUT /v1/stage/1
    updateStageById(stage:Stage):Observable<any>{
        let headers = this.getHeaders();

        return this._authHttp.put(
            this._globalService.apiHost+'/stage/'+stage.id,
            JSON.stringify(stage),
            {
                headers: headers
            }
    ...
    getAllStages(): Observable<Stage[]> {
        ...
        return this._authHttp.get(
            this._globalService.apiHost+'/stage?sort=-id',
            {
                headers: headers
            }
        )
            .map(response => response.json())
            .map((response) => {
                return <Stage[]>response.data;
            })
            .catch(this.handleError);
    }

    // GET /v1/stage
    getAllKids(): Observable<Parent[]> {
        let headers = this.getHeaders();

        return this._authHttp.get(
                this._globalService.apiHost+'/stage/kids',
            {
                headers: headers
            }
            )
